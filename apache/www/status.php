    ...
    <table class="table">
        ...
        <tbody>
            <tr v-for="submission in list">
                ...
                <td><a v-bind:style="submission.underline" v-bind:href="submission.problemUrl">{{submission.Title}}</a></td>
                <td>{{submission.Language}}</td>
                <td v-if="!submission.own"><span v-bind:class="submission.color">{{submission.Status}}</span></td>
                <td v-if="submission.own"><a v-bind:href="submission.url" v-bind:class="submission.color">{{submission.Status}}</a></td>
    ...
    var update = function(app){
        ...
        .then(response=>{
            ...
            response.data.data.submissions.forEach(item=>{
                if(!log.nologin&&(item.Name==log.Name||log.Auth)) {item.own = 1; item.url = "/viewResult?id=" + item.ID;}
                else item.own = 0;
                if(item.Status == 2) item.color = "green";
                else if(item.Status == 1) item.color = "blue";
                else if(item.Status == 0 || item.Status == 12) item.color = "orange";
                else item.color = "red";
                item.Status = ["Pending", "Running", "AC", "PE", "TLE", "MLE", "WA", "RE", "OLE", "CE", "SE", "NA", "Queuing"][item.Status];
