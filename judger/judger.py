...
def compile_code(file_name, language):
	compilers = {
		...
		"JAVA"   : "javac %s",
		"PYTHON2": '''python -c "import py_compile; py_compile.compile('%s', '%s_')"''',
...
def get_from_mysql(q, que):
	...
	db = pymysql.connect("localhost", "database", "passwd", "oj")
	...
	while True:
		try:
			...
			for i in cur.fetchall():
				cur.execute(f"UPDATE `Solutions` SET `Status`=12 WHERE `ID`={i[0]}")
				q.put(i)
